...
- (id)initWithFrame:(CGRect)frame andStars:(NSUInteger)_numberOfStars isFractional:(BOOL)isFract delegate:(id<DLStarRatingDelegate>)aDelegate {
    self.delegate = aDelegate;
    return [self initWithFrame:frame andStars:_numberOfStars isFractional:isFract];
}

- (void)setupView {
	self.clipsToBounds = YES;
	currentIdx = -1;
    if ([delegate respondsToSelector:@selector(defaultStar)]) {
        star = [[delegate defaultStar] retain];
    } else {
        star = [[UIImage imageByName:@"rating.png"] retain];
    }
	
    if ([delegate respondsToSelector:@selector(highlightedStar)]) {
        highlightedStar = [[delegate highlightedStar] retain];
    } else {
        highlightedStar = [[UIImage imageByName:@"rating_p.png"] retain];
    }
