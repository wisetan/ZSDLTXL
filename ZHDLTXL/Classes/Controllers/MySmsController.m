...
@interface MySmsController ()

@end

@implementation MySmsController
...
- (NSDictionary *)urlDictForRefresh {  
    ...
    [dict setValue:@"getSMSPeopers.json" forKey:@"path"];
...
- (UIViewController *)tableView:(UITableView *)_tableView pushRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    SmsChatViewController *talk = [[[SmsChatViewController alloc] initWithNibName:@"SmsChatViewController" bundle:nil] autorelease];
    talk.fid = [dict objForKey:@"userid"];
    talk.tel = [dict objForKey:@"tel"];
    NSLog(@"talk.fid %@", talk.fid);
    NSString *avatarUrl = [dict objForKey:@"picturelinkurl"];
    talk.fAvatarUrl = avatarUrl;
    
//    MyInfoCell *cell = (MyInfoCell *)[_tableView cellForRowAtIndexPath:indexPath];
//    cell.labInfoCount.text = @"";
